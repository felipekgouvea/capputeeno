...
export function getFieldByPriority(type: PriorityTypes) {
  if (type === PriorityTypes.NEWS) return { field: "created_at", order: "ASC" };
  if (type === PriorityTypes.BIGGEST_PRICE)
    return { field: "price_in_cents", order: "DCS" };
  if (type === PriorityTypes.MINOR_PRICE)
